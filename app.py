    ...
    if not exists(path):
        return (
            render_template(
                ...
                name="Not Found",
                code=404,
                description=f"Huh, it seems like '{str(path.absolute())}' does not exists.",
            ),
            404,
        )
    if isdir(path):
        ...
        return render_template(
            "index.html", cwd=_path, crumbs=filter_crumb(_path), files=files
        )
...
def http_error(exc: HTTPException):
    return render_template(
        "error.html", name=exc.name, code=exc.code, description=exc.description
    )
...
if app.config.get("debug", False) is False:
    ...
    def main_error(_: Exception):
        ...
        return render_template(
            "error.html", name=name, code=code, description=description
        )
